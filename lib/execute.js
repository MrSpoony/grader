...
const evaluateTestcase = async (testcase, file, submission_id, timeout) => {
    let stdout;
    let stderr;
    try {
        const command = "echo '" + testcase.input + " ' | timeout -v " + timeout + " " + file;
        ...
        stdout = e.stdout;
        stderr = e.stderr;
        if (stderr.includes("timeout: sending signal TERM to command")) {
            console.log("TLE");
            await TestcaseStatus.create({
                status_id: 6,
                testcase_id: testcase.id,
